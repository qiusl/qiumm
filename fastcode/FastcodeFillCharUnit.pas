...
var
  FillChar: FastcodeFillCharFunction = nil;
...
begin
  if isSSE3 in CPU.InstructionSupport then
    FillChar := FastcodeFillCharSSE2 {Processor Supports SSE3}
  else
    if isSSE2 in CPU.InstructionSupport then
      FillChar := FastcodeFillCharSSE2 {Processor Supports SSE2}
    else
      if isSSE in CPU.InstructionSupport then
        FillChar := FastcodeFillCharSSE {Processor Supports SSE}
      else
        if isMMX in CPU.InstructionSupport then
          FillChar := FastcodeFillCharMMX {Processor Supports MMX}
        else
          FillChar := FastcodeFillCharIA32; {Processor does not Support MMX or SSE}
end;
